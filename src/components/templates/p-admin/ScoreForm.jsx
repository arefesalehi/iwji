    ...
    <div className="bg-white m-auto pb-20 rounded-lg w-[90%]">
      ...
      {message && (
        <p
          className={`mb-4 mx-16 p-3 rounded ${
            message.includes('موفقیت') ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'
          }`}
        >
          {message}
        </p>
      ...
      <form onSubmit={handleSubmit} className="space-y-4 mb-8">
        ...
        <div className="gap-4 grid grid-cols-1 md:grid-cols-3 m-auto w-[90%]">
          <div>
            <label className="block mb-3 font-bold text-gray-900 text-sm">دوره</label>
            <select
          ...
          <div>
            <label className="block mb-3 font-bold text-gray-900 text-sm">پارت</label>
            <select
              value={selectedPart}
          ...
          <div>
            <label className="block mb-3 font-bold text-gray-900 text-sm">دانشجو</label>
            <select
        ...
        <div className="space-y-2 m-auto w-[90%]">
          <label className="block mt-14 mb-5 font-bold text-gray-700">نمرات ماژول‌ها</label>
          <div className="gap-4 grid grid-cols-1 md:grid-cols-4">
            {[0, 1, 2, 3].map((i) => (
              <div key={i} className="flex flex-col">
                <label className="block mb-3 font-bold text-gray-900 text-sm">ماژول {i + 1}</label>
                <input
                  type="number"
                  min="0"
                  max="100"
      ...
      <div className="m-auto pb-20 w-[90%]">
        ...
        {loading ? (
          ...
          <div className="bg-gray-50 p-4 border border-gray-200 rounded">
            <p className="text-gray-500">تاریخچه‌ای برای این دانشجو در این دوره و پارت وجود ندارد.</p>
          </div>
            ...
            <table className="m-auto mb-40 w-[100%] text-gray-500 dark:text-gray-400 text-sm text-left rtl:text-right">
              <thead className="bg-gray-50 dark:bg-gray-700 border border-gray-100 text-gray-700 dark:text-gray-400 text-xs uppercase">
                <tr className="bg-gray-100">
                  <th className="p-2 border border-gray-300 text-right">تاریخ ثبت </th>
                  <th className="p-2 border border-gray-300 text-center">ماژول 1</th>
                  <th className="p-2 border border-gray-300 text-center">ماژول 2</th>
                  <th className="p-2 border border-gray-300 text-center">ماژول 3</th>
                  <th className="p-2 border border-gray-300 text-center">ماژول 4</th>
                  <th className="p-2 border border-gray-300 text-center">میانگین</th>
                  <th className="p-2 border border-gray-300 text-center">وضعیت</th>
                  <th className="p-2 border border-gray-300 text-center">حذف</th>
                </tr>
              </thead>
              <tbody>
                {(() => {
                  ...
                  return sorted.map((record, recIdx) => {
                    ...
                    const maxModules = Math.max(...sorted.map(r => (r.scores || []).length), 4);
                    ...
                    for (let k = 0; k <= recIdx; k++) {
                      ...
                      (attempt.scores || []).forEach((s, j) => {
                        ...
                        bestSoFar[j] = Math.max(bestSoFar[j] === -Infinity ? -Infinity : bestSoFar[j], v);
                      });
                    }
                    // normalize -Infinity to 0
                    for (let j = 0; j < bestSoFar.length; j++) if (bestSoFar[j] === -Infinity) bestSoFar[j] = 0;
                    ...
                    const oralModules = isAllAbove45 ? bs.map((v, idx) => (v <= 75 ? `ماژول ${idx + 1}` : null)).filter(Boolean) : [];

                    return (
                      <tr key={record._id}>
                        <td className="p-2 border border-gray-200 text-right">
                          {new Date(record.createdAt).toLocaleDateString('fa-IR', { year:'numeric', month:'long', day:'numeric', hour:'2-digit', minute:'2-digit' })}
                        </td>
                        {scores.map((s, idx) => {
                          const val = getScoreValue(s);
                          let cls = 'p-2 border border-gray-300 text-center';
                          if(record.part ===1 && idx<3 && val>60) cls += ' bg-green-100 text-green-800';
                          if(record.part ===3){
                            if(val<45) cls += ' bg-red-100 text-red-800';
                            else if(val<=75) cls += ' bg-yellow-100 text-yellow-800';
                            else cls += ' bg-green-100 text-green-800';
                          }
                          return <td key={idx} className={cls}>{val.toFixed(1)}</td>;
                        })}
                        ...
                        <td className="p-2 border border-gray-300 font-bold text-center">
                          {isPassPart1 ? 'پاس شده ✅' : isDVSPass ? 'امتحان شفاهی DVS 🎤' : oralModules.length>0 ? `شامل امتحان شفاهی: ${oralModules.join(' ، ')} 📘` : '--'}
                        </td>
      ...
      <Link href="/p-admin/scores/summary" className="bg-red-800 mx-16 mb-20 p-3 rounded-lg text-white">
