...
const Charts = ({
  ...
  const answeredTickets =
    tickets?.filter((item) => item.hasAnswer === true).length || 0

  // کامنت‌ها
  const totalComments = comments?.length || 0
  const answeredComments =
    comments?.filter((item) => item.isAnswer === true).length || 0
  ...
  const totalPercentage =
    totalItems > 0 ? ((totalAnswered / totalItems) * 100).toFixed(1) : 0
  return (
    <>
        ...
        <div className="bg-white rounded-xl basis-4/12">
          ...
          <div className="flex justify-between items-center bg-blue-200 m-auto mt-20 px-5 py-8 rounded-lg w-[85%] h-[30px]">
            ...
            <Link
              ...
              className="bg-white px-2 py-1 border-1 border-blue-500 rounded-xl text-sm"
            >
              {tickets.some((item) => item.hasAnswer === false)
                ? 'در حال پیشرفت'
                : 'تکمیل شده'}
            </Link>
          </div>
          ...
          <div className="flex justify-between items-center bg-green-200 m-auto mt-5 px-5 py-8 rounded-lg w-[85%] h-[30px]">
            ...
            <Link
              href="/p-admin/comment"
              className="bg-white px-2 py-1 border-1 border-green-400 rounded-xl text-sm"
            >
              {comments.some((item) => item.isAnswer === false)
                ? 'در حال پیشرفت'
                : 'تکمیل شده'}
            </Link>
          </div>
