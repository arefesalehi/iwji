...
const CourseCreateTable = ({ allCourses , onEdit, onDelete, deleteCourse }) => {
  ...
  const handleEdit = async (course) => {
    const { value: formValues } = await Swal.fire({
      title: 'ویرایش دوره',
      html: `
        <input id="swal-name" class="swal2-input" placeholder="نام دوره" value="${course.name || ''}">
        <input id="swal-shortName" class="swal2-input" placeholder="shortName" value="${course.shortName || ''}">
        <input id="swal-level" class="swal2-input" placeholder="سطح" value="${course.level || ''}">
        <input id="swal-language" class="swal2-input" placeholder="زبان" value="${course.language || ''}">
        <input id="swal-price" type="number" class="swal2-input" placeholder="قیمت" value="${course.price || ''}">
      `,
      focusConfirm: false,
        ...
        <table className="m-auto mb-40 w-[90%] text-gray-500 dark:text-gray-400 text-xs text-left rtl:text-right">
          <thead className="bg-gray-50 dark:bg-gray-700 text-gray-700 dark:text-gray-400 text-xs uppercase">
            <tr>
              ...
              <th scope="col" className="px-6 py-3">نام کوتاه (shortName)</th>
              <th scope="col" className="px-6 py-3">سطح</th>
              <th scope="col" className="px-6 py-3">نوع</th>
              <th scope="col" className="px-6 py-3">زبان</th>
              <th scope="col" className="px-6 py-3">مدت </th>
              <th scope="col" className="px-6 py-3">پیش‌نیاز</th>
              <th scope="col" className="px-6 py-3">تعداد جلسات</th>
              <th scope="col" className="px-6 py-3">مدت هر کلاس</th>
              <th scope="col" className="px-6 py-3">ساعات کل</th>
              <th scope="col" className="px-6 py-3">ضبط</th>
              <th scope="col" className="px-6 py-3">مدرک</th>
              <th scope="col" className="px-6 py-3">قیمت</th>
              <th scope="col" className="px-6 py-3">عملگر</th>
            </tr>
          </thead>
          <tbody>
            {allCourses.map((item) => {
              ...
              return (
                <tr key={id} className="bg-white hover:bg-gray-50 dark:bg-gray-800 dark:hover:bg-gray-600 border-gray-200 dark:border-gray-700 border-b">
                  ...
                  <td className="px-6 py-4">{typeof item.recordedCourse === 'boolean' ? (item.recordedCourse ? 'بله' : 'خیر') : (item.recordedCourse || '-')}</td>
                  <td className="px-6 py-4">{typeof item.certificate === 'boolean' ? (item.certificate ? 'دارد' : 'ندارد') : (item.certificate || '-')}</td>
                  <td className="px-6 py-4">{item.price != null ? item.price : '-'}</td>
