    ...
    <div className="flex flex-col bg-gray-100 mr-2 pb-[14px] rounded-[10px] w-full min-h-[110px]">
        ...
        <div className="flex">
          ...
          <span className="flex mr-3">
            ...
            {new Array(5 - score).fill(0).map((_, index) => (
              <AiOutlineStar key={`empty-${index}`} className="text-yellow-400" />
            ))}
          </span>
        </div>
      ...
      {replies.length > 0 && (
        <div className="bg-gray-200 m-auto mt-3 p-3 rounded-xl w-[85%]">
          {replies.map((reply) => (
            <div key={reply._id} className="mb-2 pb-2 border-gray-300 last:border-0 border-b">
              <div className="flex justify-between">
                <span className="font-semibold">{reply.author?.name}</span>
                <span>{new Date(reply.createdAt).toLocaleDateString('fa-IR')}</span>
              </div>
