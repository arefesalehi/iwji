...
const RegisterCourse = ({ courses, user }) => {
  ...
  const registerHandler = async () => {
    ...
    try {
      ...
      formData.append(
        'birthDate',
        birthDate.format ? birthDate.format('YYYY-MM-DD') : birthDate,
      )
      ...
      if (birthCertificateImage)
        formData.append('birthCertificateImage', birthCertificateImage)
      ...
      if (res.status === 201) {
        ...
        swal({
          title: data.message || 'خطا در ثبت نام',
          icon: 'error',
          buttons: 'ok',
        })
      }
      ...
      console.error(err)
      swal({
        title: err.message || 'خطا در ثبت نام',
        icon: 'error',
        buttons: 'ok',
      })
    } finally {
  ...
  if (!user) {
    return (
      <>
        <div className="flex justify-center items-center h-[500px] font-bold text-xl">
          {' '}
          {t('registerCourse.first')}{' '}
          <Link href="/login-register" className="text-red-800">
            {t('registerCourse.login')}
          </Link>{' '}
          {t('registerCourse.set')}{' '}
        </div>
      </>
    )
  }
  ...
  return (
    <div className="mx-auto mb-20 px-4 md:px-8 lg:px-16 pt-20 container">
      ...
      <div className="gap-5 grid grid-cols-1 sm:grid-cols-2 bg-white dark:bg-gray-800 shadow mb-6 p-5 rounded-xl">
        <InputField
          label={t('registerCourse.birthCertificateNumber')}
          icon={<FaIdCard />}
          value={birthCertificateNumber}
          setValue={setBirthCertificateNumber}
        />
        <InputField
          label={t('registerCourse.nationalId')}
          icon={<AiFillIdcard />}
          value={nationalId}
          setValue={setNationalId}
        />
        <InputField
          label={t('registerCourse.birthPlace')}
          icon={<PiCityFill />}
          value={birthPlace}
        ...
        <InputField
          label={t('registerCourse.homeAddress')}
          icon={<FaAddressBook />}
          value={homeAddress}
          setValue={setHomeAddress}
        />
        <InputField
          label={t('registerCourse.homePostalCode')}
          icon={<MdOutlinePostAdd />}
          value={homePostalCode}
          setValue={setHomePostalCode}
        />
        <InputField
          label={t('registerCourse.workAddress')}
          icon={<FaAddressBook />}
          value={workAddress}
          setValue={setWorkAddress}
        />
        <InputField
          label={t('registerCourse.workPostalCode')}
          icon={<MdOutlinePostAdd />}
          value={workPostalCode}
          setValue={setWorkPostalCode}
        />
        <InputField
          label={t('registerCourse.phoneNumber')}
          icon={<AiFillPhone />}
          value={phoneNumber}
      ...
      <div className="gap-5 grid grid-cols-1 sm:grid-cols-2 bg-white dark:bg-gray-800 shadow mb-6 p-5 rounded-xl">
        <FileField
          label={t('registerCourse.personalPhoto')}
          setFile={setPersonalPhoto}
        />
        <FileField
          label={t('registerCourse.birthCertificateImage')}
          setFile={setBirthCertificateImage}
        />
        <FileField
          label={t('registerCourse.passportImage')}
          setFile={setPassportImage}
        />
      ...
      <button
        ...
        disabled={loading}
        className={`bg-red-800 mb-20 px-5 py-3 rounded-xl text-white w-full sm:w-auto ${
      ...
      {isRegister && RegisterData && (
        <div className="bg-gray-100 dark:bg-gray-700 shadow mt-20 mb-20 p-5 rounded-xl w-full h-auto">
          ...
          <p>
            {t('registerCourse.name')} {user.name}
          </p>
          <p>
            {t('registerCourse.phone')} {RegisterData.phoneNumber}
          </p>
          <p>
            {t('registerCourse.email')} {user.email}
          </p>
        </div>
      )}
    </div>
...
const InputField = ({ label, icon, value, setValue }) => (
  <div className="w-full">
    ...
    <div className="relative mb-4">
      <div className="absolute inset-y-0 flex items-center ps-3.5 start-0">
        {icon}
      </div>
      <input
        type="text"
